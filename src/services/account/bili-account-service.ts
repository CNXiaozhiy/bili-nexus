...
export default class BiliAccountService {
  ...
  public static init() {
    BiliAccountService.accountCheckInterval = setInterval(BiliAccountService.checkAccounts, 60 * 60 * 1000);
  ...
  public static registerOrUpdate(newAccount: UserAccount) {
    const old = this.getBiliAccount(newAccount);

    if (!old) {
      logger.info("registerOrUpdate: 账号未在Service中注册, 采用注册替代更新, UID:", newAccount.getUid());
      return this.register(newAccount);
    }

    old.getAccount().update(newAccount.getCookie(), newAccount.getRefreshToken());
    logger.info("registerOrUpdate: 更新完成, UID:", newAccount.getUid());
  ...
  public static registerDefault(account: UserAccount) {
    ...
    return (BiliAccountService.DefaultBiliAccount = BiliAccountService.register(account));
  }

  public static getDefault() {
    if (!BiliAccountService.DefaultBiliAccount) throw new BiliAccountServiceError("No default account registered");
  ...
  public static getBiliAccount(account: UserAccount | number) {
    return BiliAccountService.BiliAccounts.find(
      (instance) => instance.getAccount().getUid() === (typeof account === "number" ? account : account.getUid()),
    );
  }
    ...
    BiliAccountService.getBiliAccounts().forEach(async (account) => {
      ...
      try {
        ...
        if (checkCookieResult.code === -101 || checkCookieResult.data.refresh) {
          ...
          const { cookie, refresh_token } = await biliApi.refreshCookie(userAccount.getRefreshToken());
        ...
        notifyEmitter.emit("msg-warn", `账号 ${uid} 已失效且刷新失败❌\n\n` + FormatUtils.formatErrorMessage("ACCOUNT_REFRESH_ERROR", e));
        failedAccount.push(uid);
      }
    });

    if (failedAccount.includes(BiliAccountService.getDefault().getAccount().getUid())) {
      logger.error(`致命错误: 主(默认)账号失效且刷新失败，请立即处理 !!`);
      notifyEmitter.emit("msg-error", `主账号已失效且刷新失败❌\n\n请立即处理 !!`);
    }
  }
}
