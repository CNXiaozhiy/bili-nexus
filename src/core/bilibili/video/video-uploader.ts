  ...
  public async upload(): Promise<DoneResponse> {
    ...
    try {
      ...
      for (const video of this.options.videos) {
        ...
        this.logger.debug(`使用 endpoint -> ${endpoint}`);
            ...
            limit(async () => {
              try {
                ...
                if (totalChunks === 0) {
                  ...
                  task.process(`${(((i + 1) / totalChunks) * 100).toFixed(2)}%`);
                }
      ...
      if (!tags) {
        try {
          ...
          tags = resp
            ...
            .map((item) => item.tag)
            .join(",");
          this.logger.info("投稿标签预测成功 ✅ ->", tags);
        } catch (e) {
          this.logger.warn("投稿标签预测失败", e);
        }
      }
      ...
      this.logger.info(`投稿器[${this.name}] -> 视频 投稿完成 ->`, resp);
      ...
      if (videoInfo.season) {
        ...
        try {
          ...
          if (videoInfo.season && videoInfo.season.sectionId) {
            ...
            let season = seasonResp.seasons?.find(videoInfo.season.seasonId ? (e) => e.season.id === videoInfo.season?.seasonId : (e) => e.season.title === videoInfo.season?.name);
      ...
      const res = {
        tracker: new VideoTracker(this.name + "." + resp.bvid, this.biliAccount, resp.bvid, true),
        warnings,
        ...resp,
        bizIds,
        tasks: this.getTasks(),
